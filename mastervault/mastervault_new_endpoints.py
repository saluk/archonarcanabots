...
class MasterVault:
    ...
    def _build_cards(self, card_list, card_dict=None):
        if not card_dict:
            card_dict = {}
    ...
    def get_deck_data(self, deck_id):
        args = {
            "page": 1,
            "page_size": 1,
            "power_level": "0,11",
        ...
        dat, proxy = self._call(f"decks/{deck_id}", args)
        if "data" not in dat:
        ...
        if not dat["data"]:
            return None
        deck_data = dat["data"]
        card_data = _build_cards(dat["_linked"]["cards"])
    ...
    def get_decks_with_cards(self, direction, page, lang="en-us"):
        ...
        cards_by_key = {}
        for deck in decks:
            deck_data, card_dict = self.get_deck_data(deck["id"])
            cards_by_key.update(card_dict)
        cards_by_key_exp = {}
